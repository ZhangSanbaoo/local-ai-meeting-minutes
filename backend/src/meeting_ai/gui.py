...
class AudioPlayer(ft.Container):
    ...
    def __init__(self, on_seek: Callable | None = None, on_position_change: Callable | None = None):
        ...
        self._loop_id = 0  # 循环 ID，用于识别当前有效的循环
    ...
    async def _async_play(self):
        """异步播放（从当前位置继续）"""
        if self._audio:
            # 先 seek 到当前位置，再 resume，确保不会从头播放
            await self._audio.seek(ft.Duration(milliseconds=self.current_position_ms))
    ...
    async def _async_seek(self, position_ms: int):
        """异步跳转"""
        if self._audio:
            await self._audio.seek(ft.Duration(milliseconds=position_ms))

    async def _async_seek_and_play(self, position_ms: int):
        """异步跳转并播放"""
        if self._audio:
            # 先暂停，再 seek，再 resume（不是 play，play 会从头开始）
            await self._audio.pause()
            await self._audio.seek(ft.Duration(milliseconds=position_ms))
            await self._audio.resume()
    ...
    def seek_and_play(self, position_ms: int):
        """跳转到指定位置并播放"""
        self.current_position_ms = max(0, min(position_ms, self.duration_ms))
        if self._audio and self.audio_path and self.page:
            self.is_playing = True
            self.play_btn.icon = ft.Icons.PAUSE
            self.page.run_task(self._async_seek_and_play, self.current_position_ms)
    ...
    def _start_update_loop(self):
        ...
        self._loop_id += 1
        if self.page:
            self.page.run_task(self._async_update_loop, self._loop_id)

    async def _async_update_loop(self, my_loop_id: int):
        """异步进度更新循环"""
        while self.is_playing and my_loop_id == self._loop_id:
            # 尝试获取实际播放位置
            try:
                real_pos = await self._async_get_position()
                if real_pos > 0:
                    self.current_position_ms = real_pos
                else:
                    # 获取不到时使用估算
    ...
    def _on_position_changed(self, e):
        ...
        if e.data:
            try:
                self.current_position_ms = int(e.data)
                self._update_time_display()
                if self.on_position_change:
                    self.on_position_change(self.current_position_ms)
    ...
    def _on_state_changed(self, e):
        """flet-audio 状态变化回调"""
        if e.data:
            state = e.data.lower() if isinstance(e.data, str) else str(e.data).lower()
            if state in ("playing", "resumed"):
                self.is_playing = True
                self.play_btn.icon = ft.Icons.PAUSE
            elif state in ("paused", "stopped", "completed"):
                self.is_playing = False
                self.play_btn.icon = ft.Icons.PLAY_ARROW
                if state == "completed":
                    self.current_position_ms = 0
...
class MeetingAIApp:
    ...
    def _switch_tab(self, index: int):
        ...
        # 切换到实时录音 Tab 时，暂停音频播放
        if index == 0 and self.audio_player and self.audio_player.is_playing:
    ...
    def _build_realtime_tab(self):
        ...
        # VAD 指示器
        self.vad_indicator = ft.Container(
            content=ft.Icon(ft.Icons.MIC_NONE, size=20, color=ft.Colors.GREY_500),
            bgcolor=ft.Colors.GREY_200,
            border_radius=20,
            padding=8,
            tooltip="语音活动检测",
        ...
        # 当前说话人
        self.current_speaker_text = ft.Text(
            "等待开始录音...",
            size=14,
            color=ft.Colors.GREY_500,
        )
        ...
        # 实时转写列表
        self.realtime_list = ft.ListView(
            expand=True,
            spacing=8,
            auto_scroll=True,
            padding=10,
        ...
        # 后处理选项
        self.rt_cb_naming = ft.Checkbox(label="智能命名", value=True)
        self.rt_cb_correction = ft.Checkbox(label="错别字校正", value=True)
        self.rt_cb_summary = ft.Checkbox(label="会议总结", value=True)

        # 后处理进度
        self.rt_progress_bar = ft.ProgressBar(value=0, visible=False, width=300)
        self.rt_progress_text = ft.Text("", size=12, color=ft.Colors.GREY_600)
            ...
            content=ft.Column([
                ...
                ft.Container(
                    content=ft.Row([
                        ...
                        ft.Container(width=30),
                        ft.Column([
                            ft.Text("音量", size=10, color=ft.Colors.GREY_500),
                            self.volume_bar,
                        ], spacing=2),
                ...
                # 当前说话人
                ft.Container(
                    content=self.current_speaker_text,
                    padding=ft.Padding.symmetric(horizontal=20, vertical=8),
                ),

                # 实时转写区域
                ft.Container(
                    content=self.realtime_list,
                    border=ft.Border.all(1, ft.Colors.GREY_300),
                    border_radius=8,
                    expand=True,
                    margin=ft.Margin(left=15, right=15, top=0, bottom=10),
                ),

                # 底部选项和进度
                ft.Container(
                    content=ft.Row([
                        ft.Text("录音结束后处理:", size=12, color=ft.Colors.GREY_600),
                        self.rt_cb_naming,
                        self.rt_cb_correction,
                        self.rt_cb_summary,
                        ft.Container(expand=True),
                        self.rt_progress_bar,
                        self.rt_progress_text,
    ...
    def _on_record_clicked(self, e):
        """录音按钮点击"""
        if not self.is_recording:
            self.page.run_task(self._start_streaming)
        else:
            self.page.run_task(self._stop_streaming)
    ...
    async def _start_streaming(self):
        ...
        try:
            ...
            try:
                ...
                self._show_snackbar("需要安装 sounddevice: pip install sounddevice", error=True)
                return

            # 导入流式管道
            streaming_mod = _import_module("services.streaming")
            config = _import_module("config")

            # 重置状态
            self.realtime_segments = []
            self.realtime_list.controls.clear()

            # 创建新的管道实例
            self.streaming_pipeline = streaming_mod.reset_streaming_pipeline()
            self.streaming_pipeline.on_status_update = self._on_streaming_status
            self.streaming_pipeline.on_new_segment = self._on_streaming_segment
            ...
            self.current_speaker_text.value = "正在录音..."
            self.current_speaker_text.color = ft.Colors.GREEN_700
            self.page.update()

            # 启动录音
            output_dir = config.get_settings().paths.output_dir
            await self.streaming_pipeline.start(output_dir)

        except Exception as ex:
            import traceback
            traceback.print_exc()
            self._show_snackbar(f"启动录音失败: {ex}", error=True)
            self.is_recording = False
    ...
    async def _stop_streaming(self):
        ...
        try:
            ...
            self.current_speaker_text.value = "正在停止录音..."
            self.page.update()

            # 停止录音
            wav_path, segments = await self.streaming_pipeline.stop()
            self.realtime_segments = segments

            self.is_recording = False
            self._reset_record_btn()
            self.current_speaker_text.value = "录音已停止，开始后处理..."
            self.current_speaker_text.color = ft.Colors.ORANGE_700
            self.page.update()

            # 后处理
            await self._post_process_streaming(wav_path, segments)

        except Exception as ex:
            import traceback
            traceback.print_exc()
            self._show_snackbar(f"停止录音失败: {ex}", error=True)
            self.is_recording = False
            self._reset_record_btn()
    ...
    def _on_streaming_status(self, status):
        """流式状态更新回调"""
        # 更新时长
        mins = int(status.duration_seconds // 60)
        secs = int(status.duration_seconds % 60)
        self.duration_text.value = f"{mins:02d}:{secs:02d}"

        # 更新音量条 (-60dB ~ 0dB -> 0 ~ 1)
        volume_norm = max(0, min(1, (status.volume_db + 60) / 60))
        self.volume_bar.value = volume_norm

        # 更新音量条颜色
        if volume_norm > 0.7:
            self.volume_bar.color = ft.Colors.RED_400
        elif volume_norm > 0.4:
            self.volume_bar.color = ft.Colors.YELLOW_600
        else:
            self.volume_bar.color = ft.Colors.GREEN_400

        # 更新 VAD 指示器
        if status.is_speech:
            self.vad_indicator.bgcolor = ft.Colors.GREEN_400
            self.vad_indicator.content.color = ft.Colors.WHITE
        else:
            self.vad_indicator.bgcolor = ft.Colors.GREY_200
            self.vad_indicator.content.color = ft.Colors.GREY_500

        # 更新当前说话人
        if status.segment_count > 0:
            self.current_speaker_text.value = f"当前说话人: {status.current_speaker} | 已转写 {status.segment_count} 段"

        if self.page:
            self.page.update()

    def _on_streaming_segment(self, segment):
        """新转写片段回调"""
        # 格式化时间
        time_str = segment.format_time()
        ...
        self.realtime_list.controls.append(card)

        if self.page:
            self.page.update()

    async def _post_process_streaming(self, wav_path: Path, segments: list):
        """录音后处理"""
        try:
            self.rt_progress_bar.visible = True
            self.rt_progress_bar.value = 0
            self.rt_progress_text.value = "准备后处理..."
            self.page.update()
            ...
            summary_mod = _import_module("services.summary")
            ...
            # 1. 完整说话人分离
            self.rt_progress_bar.value = 0.1
            self.rt_progress_text.value = "说话人分离..."
            self.page.update()

            diar_service = services.get_diarization_service()

            # 在线程池中运行
            loop = asyncio.get_running_loop()
            diar_result = await loop.run_in_executor(
                None, lambda: diar_service.diarize(wav_path)
            )

            # 2. 将实时 segments 与 diarization 结果匹配
            self.rt_progress_bar.value = 0.3
            self.rt_progress_text.value = "匹配说话人..."
            self.page.update()
            ...
            for seg in segments:
                ...
                for diar_seg in diar_result.segments:
                    if diar_seg.start <= seg_mid <= diar_seg.end:
                        matched_speaker = diar_seg.speaker
                        break

                if matched_speaker is None:
                    matched_speaker = seg.temp_speaker

                final_segments.append(models.Segment(
                    id=seg.id,
                    start=seg.start,
                    end=seg.end,
                    text=seg.text,
            ...
            # 3. 合并相邻片段
            final_segments = alignment.merge_adjacent_segments(final_segments)

            # 4. 错别字校正
            if self.rt_cb_correction.value and use_llm:
                self.rt_progress_bar.value = 0.5
                self.rt_progress_text.value = "错别字校正..."
                self.page.update()
                try:
                    final_segments = await loop.run_in_executor(
                        None, lambda: correction.correct_segments(final_segments)
                    )
                except Exception:
                    pass
            ...
            # 5. 智能命名
            speakers = {}
            if self.rt_cb_naming.value:
                self.rt_progress_bar.value = 0.7
                self.rt_progress_text.value = "智能命名..."
                self.page.update()
                try:
                    gender_map = await loop.run_in_executor(
                        None, lambda: gender.detect_all_genders(wav_path, final_segments)
                    )
                    naming_service = naming.get_naming_service()
                    speakers = await loop.run_in_executor(
                        None, lambda: naming_service.name_speakers(final_segments, gender_map)
                    )
                except Exception:
                    pass
            ...
            if not speakers:
                ...
                for spk_id in speaker_ids:
                    ...
                    speakers[spk_id] = models.SpeakerInfo(
                        id=spk_id,
                        display_name=spk_id,
            ...
            # 6. 会议总结
            summary_md = ""
            if self.rt_cb_summary.value and use_llm:
                self.rt_progress_bar.value = 0.85
                self.rt_progress_text.value = "生成总结..."
                self.page.update()
                try:
                    total_duration = final_segments[-1].end if final_segments else 0
                    summary_obj = await loop.run_in_executor(
                        None, lambda: summary_mod.summarize_meeting(final_segments, speakers, duration=total_duration)
                    )
                    if summary_obj:
                        summary_md = summary_mod.format_summary_markdown(summary_obj, speakers, duration=total_duration)
                except Exception:
                    summary_md = "*总结生成失败*"

            # 7. 保存结果
            self.rt_progress_bar.value = 0.95
            self.rt_progress_text.value = "保存结果..."
            self.page.update()

            from datetime import datetime
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            output_dir = settings.paths.output_dir / f"realtime_{timestamp}"
            output_dir.mkdir(parents=True, exist_ok=True)

            # 移动录音文件
            import shutil
            final_wav = output_dir / "recording.wav"
            shutil.move(str(wav_path), str(final_wav))

            # 保存数据
            self.segments = [s.model_dump() for s in final_segments]
            self.speakers = {k: v.model_dump() for k, v in speakers.items()}
            self.result = {
            ...
            # 完成
            self.rt_progress_bar.value = 1.0
            self.rt_progress_text.value = "完成!"
            self.current_speaker_text.value = f"处理完成！共 {len(final_segments)} 个片段"
            self.current_speaker_text.color = ft.Colors.GREEN_700
            self.page.update()

            # 更新历史记录
            self._load_history()
            self._show_snackbar(f"录音已保存到: {output_dir.name}")

            # 隐藏进度条
            await asyncio.sleep(2)
            self.rt_progress_bar.visible = False
            self.rt_progress_text.value = ""
            self.page.update()

        except Exception as ex:
            import traceback
            traceback.print_exc()
            self._show_snackbar(f"后处理失败: {ex}", error=True)
            self.rt_progress_bar.visible = False
            self.rt_progress_text.value = ""
            self.current_speaker_text.value = "处理失败"
            self.current_speaker_text.color = ft.Colors.RED_700
            self.page.update()
    ...
    def _on_segment_click(self, segment_id: int):
        ...
        if 0 <= segment_id < len(self.segments):
            ...
            self.audio_player.seek_and_play(start_ms)
            self._display_segments()
