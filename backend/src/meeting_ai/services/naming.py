...
class NamingService:
    ...
    def _validate_names_with_llm(self, candidate_names: list[str], context: str) -> list[str]:
        ...
        prompt = f"""这是语音转写文本，其中可能有ASR识别错误。请严格判断以下候选词是否是真实的中文人名。
...
判断标准：
- 必须是真实的人名、昵称或姓名+职称（如"张教授"、"小柔"、"老王"）
- ASR转写错误导致的乱码词必须排除（如"小感观众"、"他工作中"不是人名）
- 普通词语、短句片段不算人名（如"小程序"、"老师"）
- 拿不准的宁可排除，不要勉强算作人名
