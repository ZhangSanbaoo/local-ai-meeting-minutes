...
class NamingService:
    ...
    def llm(self):
        """懒加载 LLM 模型"""
        if self._llm is None and self._settings.enabled:
    ...
    def _load_llm(self):
        ...
        if model_path is None:
            raise ValueError("LLM 模型路径未配置，请设置 MEETING_AI_LLM__MODEL_PATH")
        ...
        llm = Llama(
            ...
            n_gpu_layers=self._settings.n_gpu_layers,  # -1 = 全部放 GPU
            verbose=False,
        )
